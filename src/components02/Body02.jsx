...
//    const [text_State변수, setText_set함수] = useState("State변수 초기값");
...
export default function Body(props) {
  ...
  const onIncrease2 = ()=> {
    ...
    if (check%5==0) {
      ...
      // setCount(check); -> check 가 0 값으로 초기화 되고 담겨지므로 count 값도 0
      setCount(count+5);
      console.log(`** onIncrease2 if setCount 후 count=${count}`);
      //==> count+5 증가보다, console 로그가 먼저 찍혀서 정확히 나오지는 않고 있음. 
      //    (count가 증가되서 찍혀야하는데 증가가 안됨)
    } //if
  } //onIncrease2

  const onDecrease2 = ()=> {
    check--;
    console.log(`** onDecrease2 check=${check}, check=${check}`);
    if (check%5==0) {
      //=> 랜더링이 일어나도록함.
      console.log(`** onDecrease2 if setCount 전 count=${count}`);
      // setCount(check);
      setCount(count-5);
      console.log(`** onDecrease2 if setCount 후 count=${count}`);
    } //if
  } //onIncrease2
  //=======================================================
  // ** 다양한 input Tag
  ...
  return (
    <div className="body">
      ...
      <br></br>
      <p>2. input</p>
      {/* <input /><span>가나다라</span> -> 비교구문 
        => 비교구문: test 라는 변수의 영향을 받지 않는 코드이므로 input Tag 에 입력값이 표시됨
        */}
      <input value={text} onChange={textChange} />&nbsp;&nbsp;&nbsp;&nbsp;<span>{text}</span>
      {/* => input 에서 키보드 입력해도 화면에 표시되지 않음.
          => 키보드 누를때마다 text 변수값을 변경 시켜야 되고 (이벤트코드 필요함)
             출력되려면 랜더링이 일어나야 함(상태변수 필요) */}
      <br></br>
