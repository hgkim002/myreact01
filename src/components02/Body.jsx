...
export default function Body() {
  ...
  const onClickBtn = ()=>{
    ...
    => 이 값은 DOM 엘리먼트에 직접 반영
      화면에 출력은 되지만, 
      랜더링은일어나지 않았고, 상태변수에도 영향을 주지않음
    => document.getElementById('...').value 접근과 동일. 실제 DOM 을 수정.
    */
    ...
    if (text.length<3) {
      ...
      // textRef 로 값을 초기화해서 화면에서는 초기화가 되지만, 상태변수 text 값은 그대로 남아있다.
    }

  } //onClickBtn

  //** 컴포넌트 랜더링(update) 확인
  console.log(`** 컴포넌트 Body 랜더링(update) !!! **`);
