...
// ** 컴포넌트에 조건문 적용
//=> JSX 의 {...} 에서는 조건문이 불가능 하지만, (삼항식 가능)
//   컴포넌트에 조건문 적용은 가능함. (JS 이므로 가능...)

//=====================================================================

// ** React Event (Html 과 차이점) 
// => 이벤트 리스너 카멜표기   (html은 모두소문자)
    ...
    return (
      <div className="body">
        ...
        {/* <p>* 논리식: boo1={boo1}, AND={boo1&&boo2}, 관계식={s1===s2}</p> 
            =>boolean 값은 toString() 메서드로 문자화 해야 출력가능
        */}
        <p>* 논리식: boo1={boo1.toString()}, AND={(boo1&&boo2).toString()}, 관계식={(s1===s2).toString()}</p> 
        <p>객체: obj.id={obj.id}, obj.name={obj.name}</p>
        {/* <p>객체: obj={obj}</p>
        컨텐츠 또는 값(리터럴) 을 표현하는 위치에서 객체, 배열명 사용은 불허 */}
        <p>* 삼항식 Test</p>
        <p>~ n1+n2 의 값이 짝수이면 "짝수", 홀수면 "홀수" 로 출력하세요 ~ </p>
      ...
      <div className="body">
        <h2 style={ {color:'white', backgroundColor:'hotpink'} }>
          ** Body: image & style & event Test</h2>
          <img src={img1} alt={"ImageTest"} style={{width:100, height:150}}></img>
          &nbsp;&nbsp;
          <button name='apple' onClick={clickTest}>Apple:콜백함수방식</button>&nbsp;&nbsp;
          <button name='banana' onClick={(event)=>{ clickTest(event) }}>Banana:직접기술방식</button>
